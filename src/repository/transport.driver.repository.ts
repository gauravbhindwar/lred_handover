...
import { ITransportDriverCreate, TransportDriverAttributes } from '@/interfaces/model/transport.driver.interface';
...
export default class TransportDriverRepo extends BaseRepository<TransportDriver> {
	constructor() {
	...
	async getAllTransportDriver(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			where: {
				...
				...(search && { driverNo: { [Op.iLike]: '%' + search.toLowerCase() + '%' } }),
			},
	...
	async getAvailableTransportDriver(query: IQueryParameters) {
		...
		for (let m = moment(transportStartDates); m.isSameOrBefore(transportEndDates); m.add(1, 'days')) {
	...
	async addTransportDriver({ body, user }: { body: ITransportDriverCreate; user: User }) {
	...
	async updateTransportDriver({ body, user, id }: { body: TransportDriverAttributes; user: User; id: number }) {
		const isExistDriver = await TransportDriver.findOne({
			where: {
				id: id,
				deletedAt: null,
		...
		await TransportDriver.update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteTransportDriver(id: number) {
		const isExistDriver = await TransportDriver.findOne({
			where: {
				id: id,
				deletedAt: null,
