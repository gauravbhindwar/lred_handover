...
export default class TimesheetScheduleRepo extends BaseRepository<TimesheetSchedule> {
	...
	async getAllTimesheetScheduleDetails(query: IQueryParameters, user: User) {
		...
		// 		// 					Sequelize.fn(
		// 		// 						'concat',
		// 		// 						Sequelize.col('loginUserData.firstName'),
		// 		// 						' ',
		// 		// 						Sequelize.col('loginUserData.lastName'),
		// 		// 					),
		// 		// 					{
		// 		// 						[Op.iLike]: `%${search}%`,
		// 		// 					},
	...
	async updateTimesheetScheduleByEmployeeId(body: {
		...
		try {
			...
			return await this.updateTimesheetScheduleById(
				...
				transaction,
				body?.overtimeHours ||
					(body.updateStatus == 'CLEARFIELD' || body.updateStatus == 'CLEARFIELDTOP' ? null : body?.overtimeHours),
			);
		} catch (error) {
			throw error;
		}
