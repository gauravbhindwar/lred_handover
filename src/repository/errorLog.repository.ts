...
export default class ErrorLogsRepo extends BaseRepository<ErrorLogs> {
	...
	async getAllErrorLogsService(query: IQueryParameters) {
		const { page, limit, clientId, sortBy, sort, startDate, endDate, type } = query;
		...
		let data = await this.getAllData({
			...
			include: [
				{ model: Client, attributes: ['loginUserId'], include: [{ model: LoginUser, attributes: ['name'] }] },
				{ model: User, attributes: ['loginUserId'], include: [{ model: LoginUser, attributes: ['name'] }] },
			],
	...
	async addErrorLogs({ body }: { body: IErrorLogsCreate }) {
		...
		await createHistoryRecord({
			tableName: tableEnum.ERROR_LOGS,
			jsonData: parse(data),
