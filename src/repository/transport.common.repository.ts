...
import { statusEnum } from '@/interfaces/model/history.interface';
import { ITransportCommonCreate, TransportCommonAttributes } from '@/interfaces/model/transport.common.interface';
...
export default class TransportCommonRepo extends BaseRepository<TransportModels> {
	constructor() {
	...
	async getAllCommonTransport(query: IQueryParameters) {
		...
		let data = await Models.models[type].findAndCountAll({
			where: {
				...
				...(search && { name: { [Op.iLike]: '%' + search.toLowerCase() + '%' } }),
			},
	...
	async addCommonTransport({ body, user }: { body: ITransportCommonCreate; user: User }) {
		...
		if (isExistTransportModels) {
			throw new HttpException(200, this.customMsg.custom(`${body.type} already exist`), {}, true);
		}
		let data = await Models.models[body.type].create({ ...body, createdBy: user.id });
		data = parse(data);
	...
	async updateCommonTransport({ body, user, id }: { body: TransportCommonAttributes; user: User; id: number }) {
		const isExistTransportModels = await Models.models[body.type].findOne({
			where: {
				id: id,
				deletedAt: null,
		...
		const isAlreadyExistTransportModels = await Models.models[body.type].findOne({
			where: {
				name: body.name,
				clientId: body.clientId,
				id: {
		...
		await Models.models[body.type].update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteCommonTransport(id: number, type: string) {
