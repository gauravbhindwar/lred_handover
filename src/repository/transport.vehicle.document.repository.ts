...
export default class TransportVehicleDocumentRepo extends BaseRepository<TransportVehicleDocument> {
	constructor() {
	...
	async getAllTransportVehicleDocument(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			where: { deletedAt: null, vehicleId: vehicleId, ...(clientId && { clientId: clientId }) },
			offset: page && limit ? (page - 1) * limit : undefined,
			limit: limit ?? undefined,
			include: [{ model: TransportVehicle, attributes: ['deletedAt'], where: { deletedAt: null } }],
	...
	async addTransportVehicleDocument({ body, user }: { body: ITransportVehicleDocumentCreate; user: User }) {
		let data = await TransportVehicleDocument.create({ ...body, createdBy: user.id });
		data = parse(data);
	...
	async updateTransportVehicleDocument({
		...
		await TransportVehicleDocument.update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteTransportVehicleDocument(id: number) {
		const isExistDocument = await TransportVehicleDocument.findOne({
			where: {
				id: id,
				deletedAt: null,
