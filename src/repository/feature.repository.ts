...
export default class FeatureRepo extends BaseRepository<Feature> {
	...
	async addFeatureService({ body, user }: { body: ICreateUpdateFeature; user: User }) {
		...
		for (const element of body.permission) {
			const isPermissionExist = await this.checkAvailablePermission(element, data.id);
	...
	async checkAvailablePermission(permission: string, id: number) {
		return await Permission.findOne({ where: { permissionName: permission, featureId: id } });
	}

	async updateFeatureService({ body, user, id }: { body: ICreateUpdateFeature; user: User; id: number }) {
		const isExist = await Feature.findOne({ where: { name: body.name, id: { [Op.ne]: id } } });
		if (isExist) {
			throw new HttpException(200, this.msg.exist, {}, true);
		}
		let data = await Feature.update({ name: body.name, updatedBy: user.id }, { where: { id: id } });
		for (const element of body.permission) {
			const isPermissionExist = await this.checkAvailablePermission(element, id);
		...
		await Permission.destroy({ where: { featureId: id, permissionName: { [Op.notIn]: body.permission } } });
