...
export default class ReliquatAdjustmentRepo extends BaseRepository<ReliquatAdjustment> {
	constructor() {
	...
	async getAllReliquatAdjustment(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			...
			include: [
				...
				{ model: Employee, attributes: ['id'], include: [{ model: LoginUser, attributes: ['firstName', 'lastName'] }] },
			],
	...
	async addReliquatAdjustment({ body, user }: { body: IReliquatAdjustmentCreate; user: User }) {
		await generateModalData({ user: user, percentage: 0, message: 'Updating Reliquat Calculation' });
		...
		if (isExist) {
			data = await ReliquatAdjustment.update(
				...
				{ where: { id: isExist.id }, transaction },
			);
		} else {
			data = await ReliquatAdjustment.create({ ...body, createdBy: user.id }, { transaction });
		}
		data = parse(data);
		await generateModalData({ user: user, percentage: 40, message: 'Updating Reliquat Calculation' });
		if (body.employeeId) {
			const lastDayOfNextMonth = moment().clone().add(2, 'month').endOf('month');
			...
			await generateModalData({ user: user, percentage: 100, message: 'Updating Reliquat Calculation' });
		...
		data = await this.getReliquatAdjustmentById(isExist ? isExist.id : data.id, transaction);
	...
	async updateReliquatAdjustment({ body, user, id }: { body: IReliquatAdjustmentCreate; user: User; id: number }) {
		await generateModalData({ user: user, percentage: 0, message: 'Updating Reliquat Calculation' });
		const transaction = await db.transaction();
		...
		await ReliquatAdjustment.update(
			...
			{ where: { id: id, deletedAt: null }, transaction },
		);
		await generateModalData({ user: user, percentage: 40, message: 'Updating Reliquat Calculation' });
	...
	async deleteReliquatAdjustment(id: number, user: User) {
		...
		await generateModalData({ user: user, percentage: 40, message: 'Updating Reliquat Calculation' });
		const data = await ReliquatAdjustment.update({ updatedBy: user.id, deletedAt: new Date() }, { where: { id: id } });
