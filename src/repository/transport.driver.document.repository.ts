...
export default class TransportDriverDocumentRepo extends BaseRepository<TransportDriverDocument> {
	constructor() {
	...
	async getAllTransportDriverDocument(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			where: { deletedAt: null, driverId: driverId, ...(clientId && { clientId: clientId }) },
			offset: page && limit ? (page - 1) * limit : undefined,
			limit: limit ?? undefined,
			include: [{ model: TransportDriver, attributes: ['deletedAt'], where: { deletedAt: null } }],
	...
	async addTransportDriverDocument({ body, user }: { body: ITransportDriverDocumentCreate; user: User }) {
		let data = await TransportDriverDocument.create({ ...body, createdBy: user.id });
		data = parse(data);
	...
	async updateTransportDriverDocument({
		...
		await TransportDriverDocument.update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteTransportDriverDocument(id: number) {
		const isExistDocument = await TransportDriverDocument.findOne({
			where: {
				id: id,
				deletedAt: null,
