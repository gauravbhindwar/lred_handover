...
export default class TimesheetRepo extends BaseRepository<Timesheet> {
	...
	async getAllTimesheetService(query: IQueryParameters, user: User) {
		const { page, limit, clientId, sortBy, sort, activeTab, startDate, endDate, segmentId, subSegmentId, search } =
			query;
		let condition = {};
		...
		const dates = {
			startDate: {
				[Op.or]: {
					[Op.between]: [
						moment(startDate, this.dateFormat).toDate(),
						moment(moment(endDate, this.dateFormat)).toDate(),
					],
					[Op.eq]: moment(moment(startDate, this.dateFormat)).toDate(),
				},
			},
			endDate: {
				[Op.or]: {
					...
					[Op.eq]: moment(moment(endDate, this.dateFormat)).toDate(),
				},
			},
		};
		if (activeTab) {
		...
		let data = await Timesheet.findAndCountAll({
			include: [
				{
					model: Segment,
					attributes: ['name', 'id'],
				...
				{
					model: Employee,
					attributes: ['id', 'loginUserId', 'contractEndDate', 'deletedAt'],
					required: true,
					paranoid: false,
					include: [
						{
			...
			where: { clientId: clientId, ...condition },
			offset: page && limit ? (page - 1) * limit : undefined,
			limit: limit ?? undefined,
			order: [
				// [sortedColumn ?? 'startDate', sort ?? 'asc'],
				['segment', 'name', 'asc'],
	...
	async getTimesheetByIdService(id: number, transaction: Transaction = null) {
		const isFound = await Timesheet.findOne({
			...
			attributes: ['id', 'segmentId', 'subSegmentId', 'employeeId', 'startDate', 'endDate', 'clientId'],
			include: [
				{
					model: Employee,
					attributes: ['id'],
	...
	async addTimesheetService({ body, user = null }: { body: ITimesheetCreate; user: User }, transaction: Transaction) {
	...
	async updateTimesheetService({
		...
		try {
			const isExist = await Timesheet.findOne({ where: { id: id, deletedAt: null }, transaction });
			if (!isExist) {
	...
	async deleteTimesheetService({ id }: { id: number }) {
		const isFound = await Timesheet.findOne({ where: { id: id, deletedAt: null } });
		if (!isFound) {
			throw new HttpException(404, this.msg.notFound);
		}
	...
	async deleteTimesheetSummaryService(deleteData, transaction: Transaction = null) {
		const data = await Timesheet.destroy({ where: { ...deleteData }, transaction });
		return data;
	}

	async sendPdf(data, emails, pdfName, title, attribute, stampLogo) {
		const resizeHeaderFooter = true;
		const itemData = data?.timesheetData?.timesheetLogsData?.map((item, index) => {
			return `Version ${index + 1} ${item?.status} by ${
				item?.actionByUser?.loginUserData?.firstName && item?.actionByUser?.loginUserData?.lastName
					? item?.actionByUser?.loginUserData?.firstName + +item?.actionByUser?.loginUserData?.lastName
					: item?.actionByUser?.loginUserData?.firstName
					? item?.actionByUser?.loginUserData?.firstName
					: item?.actionByUser?.loginUserData?.name
			} on ${moment(item?.actionDate).format('LL')} at ${moment(item?.actionDate).format('LT')}`;
		});

		data.allMonthData.forEach((dataItem, index) => {
			const dayNumber = moment(dataItem?.rawDate).day();
			const isWeekend =
				((data?.employeeData?.client.country === 'Algeria' || data?.employeeData?.client.country === 'Libya') &&
		...
		const footerContent = itemData.join(',').replaceAll('UNAPPROVED', 'unapproved').replaceAll('APPROVED', 'approved');
		...
		const replacement = {
			...
			timesheetPeriod: data.timesheetData.startDate + '-' + data.timesheetData.endDate,
	...
	async getTimesheetDropdownDetails(clientId, user: User, activeDate = null, isActiveDate = false) {
		const condition = this.generateConditionForDropdown(activeDate, isActiveDate);
		const output = {
			dates: [],
			segment: [],
			subSegment: [],
		...
		timesheetData = await Timesheet.findAll({
			...
			subQuery: false,
			attributes: [
				[Sequelize.fn('DISTINCT', Sequelize.literal('concat("segment"."id", "subSegment"."id")')), 'segmentUniqueId'],
			],
			include: [
				{
					model: Segment,
			...
			where: {
				clientId,
				...(condition ? { startDate: { [Op.lte]: condition }, endDate: { [Op.gte]: condition } } : {}),
				[Op.and]: [
					{ ...(segmentIds?.length > 0 && { segmentId: { [Op.in]: segmentIds } }) },
					{
						...(subSegmentIds?.length > 0 && {
		...
		timesheetData.forEach((timesheet) => {
			...
			if (segment && output.segment.findIndex((dat) => dat?.id == segment?.id) == -1) {
				if (!segment?.deletedAt) {
					if (!subSegment?.id) {
						output.segment.push(segment);
					}
				} else {
					if (output.deletedSection.findIndex((dat) => dat?.id == segment?.id) == -1) {
						output.deletedSection.push({ ...segment, type: 'segment' });
					}
				}
			}
			if (subSegment && output.subSegment.findIndex((dat) => dat?.id == subSegment?.id) == -1) {
				if (!subSegment?.deletedAt) {
					output.subSegment.push(subSegment);
				} else {
					if (
						output.deletedSection.findIndex(
							(dat) => dat?.id == subSegment?.id && dat?.segmentId == subSegment?.segmentId,
						) == -1
					) {
		...
		let clientData = await Client.findOne({
			...
			attributes: ['id', 'autoUpdateEndDate', 'startDate', 'endDate', 'timeSheetStartDay'],
		});
		...
		if (clientData.autoUpdateEndDate == 0 && moment().isAfter(clientData.endDate)) {
			output.isEnded = true;
		}
		return output;
	}
	...
	async approveTimesheet(
		...
		try {
			...
			await generateModalData({
				user: body?.user,
				percentage: 25,
				message: body.newStatus === 'APPROVED' ? 'Approving Timesheet' : 'Unapproving Timesheet',
			});
			await Timesheet.update(
				{
				...
				{ where: { id: body.timesheetId }, transaction },
			);

			let timesheetData = await this.getTimesheetByIdService(body.timesheetId, transaction);
			timesheetData = parse(timesheetData);
			...
			if (body.newStatus === 'APPROVED') {
				...
				if (data) {
					...
					await generateModalData({ user: body?.user, percentage: 50, message: "Generating Account PO's" });
					...
					data.timesheetData.timesheetLogsData = [...data.timesheetData.timesheetLogsData];
					const stampLogo = data?.employeeData?.client?.stampLogo
						? SERVER_URL + data?.employeeData?.client?.stampLogo
						: null;
					mailData = await this.sendPdf(data, [], `${pdfName}`, 'timesheetPdf', true, stampLogo);
				}
			}

			if (body.newStatus === 'UNAPPROVED') {
				await generateModalData({ user: body?.user as User, percentage: 75, message: 'Updating Accounts' });
							...
							date: {
								[Op.or]: {
									[Op.between]: [
										...
										moment(moment(isExist.endDate).format('DD-MM-YYYY'), 'DD-MM-YYYY').toDate(),
									],
									[Op.eq]: moment(moment(isExist.endDate).format('DD-MM-YYYY'), 'DD-MM-YYYY').toDate(),
								},
							},
	...
	async generateAccountData(user, employeeId = [], type: string, clientId = null, transaction: Transaction = null) {
		try {
			const timesheetData = await this.getTimesheetDataForReliquet(clientId, employeeId, [], transaction);
			const promises = [];
	...
	async getReliquatAdjustmentDate(clientId: number, type: string, employeeId: number) {
		let respo = await Timesheet.findOne({
			where: { clientId: clientId, employeeId: employeeId, deletedAt: null, status: 'APPROVED' },
			// attributes: [
		...
		respo = parse(respo);
		let isExistClient = await Client.findOne({ where: { id: clientId, deletedAt: null } });
			...
			if (type === 'adjustment') {
				while (startDate.isSameOrBefore(endDate)) {
					...
					startDateOfTimesheet &&
						startDateOfTimesheet.forEach((ele) => {
							ele.startDate && result.push(moment(ele?.startDate).format('YYYY-MM-DD'));
				...
				while (startDate.isSameOrBefore(endDate)) {
					...
					startOfTimesheet.forEach((ele) => {
						const currentDate = ele?.startDate && moment(ele?.startDate).startOf('day');
						const endDateOfMonth = ele?.endDate && moment(ele?.endDate).endOf('month');
						while (currentDate && endDateOfMonth && currentDate.isSameOrBefore(endDateOfMonth, 'day')) {
	...
	async getTimesheetDataForReliquet(
		...
		try {
			...
			const condition = employeeId?.length > 0 ? { employeeId: { [Op.in]: employeeId } } : { clientId };
			let resp = null;
			if (timesheetScheduleIds?.length > 0) {
				...
				if (
					...
					moment(starterDate).isSameOrBefore(reliquatCalculationLastRecord.startDate)
				) {
					resp = await Timesheet.findAll({
						...
						where: {
							...
							startDate: { [Op.lte]: moment(reliquatCalculationLastRecord.startDate).toDate() },
							...condition,
						},
						order: [['startDate', 'asc']],
						transaction,
					...
					resp = await Timesheet.findAll({
						...
						where: {
							...
							startDate: { [Op.gte]: moment(reliquatCalculationLastRecord.startDate).toDate() },
							...condition,
						},
						order: [['startDate', 'asc']],
						transaction,
				...
				resp = resp.reduce((acc, timesheet) => {
					const indexData = acc.find((e) => e.startDate === timesheet.startDate);
	...
	async getTimesheetDataForAccount(user, employeeId = [], date, transaction: Transaction = null) {
				...
				for (const tempTimeSheetData of timesheetData) {
					const promise = new Promise(async (resolve, reject) => {
						try {
							...
							// 	[tempTimeSheetData.employeeId],
							// 	tempTimeSheetData.id,
							// 	user?.id || null,
							// 	transaction,
							// );
							resolve(true);
	...
	async getAllTimesheetByEmployeeId(employeeId: number, transaction: Transaction = null) {
		try {
			const data = await Timesheet.findAll({
	...
	async clearEmployeeTimesheetByEmployeeId(employeeId: number | string, transaction: Transaction = null) {
	...
	async getTimesheetDataUsingDate(employeeId: number, date: string | Date, transaction: Transaction) {
		const timesheetData = await this.getAll({ where: { employeeId: employeeId }, transaction });
	...
	async findTimesheetSummaryById(
		id: number,
		transaction: Transaction = null,
		actualStartDate?: Date,
		actualEndDate?: Date,
		...
		try {
			...
			if (timesheetData) {
				...
				const segmentStartDate = moment(timesheetData.startDate).add(1, 'month').toDate();
				const segmentEndDate = moment(timesheetData.endDate).add(1, 'month').toDate();
				const startDate = moment(timesheetData.startDate).format('Do MMMM YYYY');
					...
					include: [
						{
							...
							attributes: ['employeeId', 'status', 'isLeaveForTitreDeConge', 'date', 'bonusCode', 'overtimeHours'],
							where: {
								date: {
				...
				if (employeeData?.length > 0) {
					...
					for (let m = moment(actualStartDate); m.isSameOrBefore(actualEndDate); m.add(1, 'days')) {
						const index = employeeData[0]?.timeSheetSchedule
							?.map((dat) => moment(dat.date).toDate())
							?.findIndex((dat) => m.isSame(dat));
						if (index >= 0) {
							const bonusCode = employeeData[0]?.timeSheetSchedule[index]?.bonusCode;
							const initialStatus = employeeData[0]?.timeSheetSchedule[index]?.status;
							const overtimeHours = employeeData[0]?.timeSheetSchedule[index]?.overtimeHours ?? null;
							const isLeaveForTitreDeConge = employeeData[0]?.timeSheetSchedule[index]?.isLeaveForTitreDeConge ?? null;
							let status = '';
							status = await this.getStatus({ overtimeHours, initialStatus, bonusCode });
					...
					for (const statusType of status) {
						const result = employeeData[0]?.timeSheetSchedule.filter((element) => {
							return element.status === statusType && element?.bonusCode === null;
						});
					...
					for (const accountType of employeeData[0].timeSheetSchedule) {
						const isExist = statusMap.get(
							`${accountType.status}${accountType.bonusCode ? ',' + accountType.bonusCode : ''}`,
						);
						if (!isExist) {
							const index = pdfStatusArr?.findIndex(
								(t) => t === `${accountType.status}${accountType.bonusCode ? ',' + accountType.bonusCode : ''}`,
							);
							if (index !== -1) {
								statusMap.set(
									`${accountType.status}${accountType.bonusCode ? ',' + accountType.bonusCode : ''}`,
					...
					for (const bonusType of bonusData) {
						const result = employeeData[0]?.timeSheetSchedule.filter((element) => {
							return element?.bonusCode?.split(',')?.indexOf(bonusType?.code) >= 0;
						});
						if (result?.length > 0) {
							const currentBonus = customBonus?.find((a) => a?.id == bonusType?.id);
							bonusCount.push({
								bonusType: bonusType.code,
								bonusName: bonusType.name,
								length: result?.length,
								basePrice:
									currentBonus && currentBonus?.label === bonusType?.code ? currentBonus.price : bonusType.basePrice,
							});
						}
					}
					const overtimeWeekendBonus: { label: string; title?: string; length: number }[] = [];
					if (employeeData[0]?.timeSheetSchedule?.length > 0) {
						if (
							employeeData[0]?.timeSheetSchedule?.findIndex((e) => e.bonusCode?.split(',')?.indexOf('O1') >= 0) >= 0
						) {
							const o1Length = employeeData[0]?.timeSheetSchedule?.filter(
								(e) => e.bonusCode?.split(',')?.indexOf('O1') >= 0,
							)?.length;
							overtimeWeekendBonus?.push({ label: 'O1', title: 'Overtime 1', length: o1Length });
						}
						if (
							employeeData[0]?.timeSheetSchedule?.findIndex((e) => e.bonusCode?.split(',')?.indexOf('O2') >= 0) >= 0
						) {
							const o2Length = employeeData[0]?.timeSheetSchedule?.filter(
								(e) => e.bonusCode?.split(',')?.indexOf('O2') >= 0,
							)?.length;
							overtimeWeekendBonus?.push({ label: 'O2', title: 'Overtime 2', length: o2Length });
						}
						if (employeeData[0]?.timeSheetSchedule?.findIndex((e) => e.bonusCode?.split(',')?.indexOf('W') >= 0) >= 0) {
							const wLength = employeeData[0]?.timeSheetSchedule?.filter(
								(e) => e.bonusCode?.split(',')?.indexOf('W') >= 0,
							)?.length;
							overtimeWeekendBonus?.push({ label: 'W', title: 'Weekend Overtime', length: wLength });
						}
						if (
							employeeData[0]?.timeSheetSchedule?.findIndex((e) =>
								hourlyBonusTypeArr?.includes(`${e.status}${e.bonusCode ? ',' + e.bonusCode : ''}`),
							) >= 0
						) {
							...
							overtimeWeekendBonus?.push({ label: 'HOB', title: 'Hourly Overtime Bonus', length: hobLength });
						}
						if (
							employeeData[0]?.timeSheetSchedule?.findIndex(
								(e) => e.bonusCode === null && e.status === 'H' && e.overtimeHours !== null,
							) >= 0
						) {
							const hobLength = employeeData[0]?.timeSheetSchedule
								?.filter((e) => e.bonusCode === null && e.status === 'H' && e.overtimeHours !== null)
								?.reduce((sum, ele) => sum + ele.overtimeHours, 0);
							overtimeWeekendBonus?.push({ label: 'H', title: 'Hourly Bonus', length: hobLength });
						}
					}
					...
					if (
						moment(employeeData[0]?.terminationDate).isBetween(momentStartDate.toDate(), momentEndDate.toDate()) ||
						moment(employeeData[0]?.terminationDate).isSame(momentStartDate.toDate()) ||
						moment(employeeData[0]?.terminationDate).isSame(momentEndDate.toDate())
					) {
					...
					} else if (employeeData[0]?.employeeSegment?.length > 0 && !employeeData[0]?.employeeSegment?.[0]?.rollover) {
						const reliquatCalculation = await this.reliquatCalculationRepo.generateReliquatCalculationService({
							employeeId: String(employeeData[0]?.id),
							date: moment(
								moment(employeeData[0]?.employeeSegment[0]?.date).subtract(1, 'day').format('DD-MM-YYYY'),
								'DD-MM-YYYY',
							).toDate(),
						});
						reliquatValue = reliquatCalculation >= 0 ? reliquatCalculation : 0;
					...
					let presentValue = this.countStatus(employeeData[0]?.timeSheetSchedule, 'P');
					presentValue += this.countStatus(employeeData[0]?.timeSheetSchedule, 'CHB');
					let CRValue = 0;
					if (employeeData[0]?.client?.isCountCR) {
						CRValue = this.countStatus(employeeData[0]?.timeSheetSchedule, 'CR');
					}
					let trValue = this.countStatus(employeeData[0]?.timeSheetSchedule, 'TR');
					let apValue = this.countStatus(employeeData[0]?.timeSheetSchedule, 'AP');
					let caValue = this.countStatus(employeeData[0]?.timeSheetSchedule, 'CA');
					...
					const totalCount =
						presentValue + CRValue + trValue + apValue + caValue + totalBonusCount + reliquatPayment + reliquatValue;
					// const totalCount =
					// 	presentValue +
					// 	CRValue +
					// 	trValue +
					...
					const isHourlyBonusNote = overtimeWeekendBonus?.some((e) => e?.label === 'HOB') ? true : false;
	...
	countStatus(allRows: TimesheetScheduleAttributes[], field: string, isBonus = false) {
		...
		return allRows.reduce((count, obj) => {
			if (
				...
				!(obj.bonusCode?.split(',').indexOf('P') >= 0 && field == 'P' && obj.status == '') &&
				field != 'TR'
			) {
				count++;
			}
			...
			// 	count++;
			// }
			if (this.getStatusName(obj, true, true)?.indexOf('TR') >= 0 && field == 'P') {
				count++;
			}
			if (isBonus) {
				if (this.getStatusName(obj, isBonus, true)?.indexOf('P') >= 0 && field == 'P') {
					count++;
				}
			}
			if (
	...
	getStatusName(rowData: TimesheetScheduleAttributes, isBonus = false, isCount = false) {
	...
	async generateAccountPO(
		...
		try {
			...
			if (timesheetData?.length > 0) {
				let poNumber = 'PO' + String(data.timesheetId) + String(Math.floor(1000 + Math.random() * 9000));
							...
							if (customBonus && customBonus?.length > 0) {
								...
								if (isExistingCustomBonus >= 0) {
									...
									filteredCustomBonus.forEach((bonusValue, index) => {
										...
										if (bonusValue?.endDate && moment(bonusValue?.endDate).subtract(1, 'days').isAfter(momentEndDate)) {
											endDate = momentEndDate;
										} else {
											endDate = moment(
												bonusValue?.endDate &&
													moment(timesheetData[0]?.employee?.terminationDate).isSame(bonusValue.endDate)
				...
				if (empSalary && empSalary?.length > 0) {
					empSalary.forEach((salaryValue, index) => {
						...
						if (salaryValue?.endDate && moment(salaryValue?.endDate).isAfter(momentEndDate)) {
							endDate = momentEndDate;
						} else {
						...
						const presentSalaryDays = timesheetData[0]?.employee?.timeSheetSchedule?.filter((itemValue) => {
							...
							return (
								...
								(itemValue.status === 'P' ||
									(timesheetData[0].client?.isCountCR && itemValue.status === 'CR') ||
					...
					if (
						hourlyOvertimeBonus.includes(
							`${timesheetDataFilter?.status}${
								timesheetDataFilter?.bonusCode ? ',' + timesheetDataFilter?.bonusCode : ''
							}`,
						)
					) {
						length = 1;
				...
				const hourlyCallOutFilteredData = timesheetData[0]?.employee?.timeSheetSchedule?.filter(
					(e) => e.status === 'H' && e.bonusCode === null && e.overtimeHours !== null,
				);

				for (const timesheetDataFilter of hourlyCallOutFilteredData) {
					const isExist = hourlyCallOutBonus.get(`${timesheetDataFilter?.status}${timesheetDataFilter?.overtimeHours}`);
					if (isExist) {
						const length = isExist.length + 1;
						hourlyCallOutBonus.set(`${timesheetDataFilter?.status}${timesheetDataFilter?.overtimeHours}`, {
				...
				const hourlyCallOutPONumber = 'PO' + String(data.timesheetId) + String(Math.floor(1000 + Math.random() * 9000));
				...
				for (const element of bonusArr) {
					...
					poNumber = 'PO' + String(data.timesheetId) + String(Math.floor(1000 + Math.random() * 9000));
					await AccountPO.create(
						{
							timesheetId: data.timesheetId,
							type: element.label,
