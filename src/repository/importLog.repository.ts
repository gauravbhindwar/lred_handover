...
import { createRandomHash, generateUniquePassword, parse } from '@/utils/common.util';
...
export default class ImportLogRepo extends BaseRepository<ImportLog> {
	...
	async findAllImportLogData(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			...
			include: [{ model: Client, attributes: ['loginUserId'], include: [{ model: LoginUser, attributes: ['name'] }] }],
	...
	async addImportEmployee({ body, user, image }: { body: IImportLogCreate; user: User; image: Express.Multer.File }) {
		try {
			if (image?.filename) body.fileName = `/employeeData/${image?.filename}`;

			const workbook = XLSX.readFile(path.join(__dirname, `../../secure-file/employeeFile/${image.filename}`));
			...
			for (const dataItem of dataValue) {
				...
				try {
					const resp = await this.checkData(dataItem, body, user, value.id, transaction);
					...
					let isExist = await Employee.findOne({
						where: { employeeNumber: dataItem?.Matricule, clientId: body.clientId, deletedAt: null },
						include: [
							{
								model: SubSegment,
								attributes: ['id'],
					...
					const isCheckEmployeeNumber = isExist?.employeeNumber != dataItem?.Matricule;
					const data = {
	...
	async checkData(data: any, body: IImportLogCreate, user: User, logId: number, transaction: Transaction = null) {
		try {
			let finalData: any = [];

			const result = await this.checkSegmentSubSegment(data, body, user, transaction);
			...
			let isExist = await Employee.findOne({
				where: { employeeNumber: data?.Matricule, clientId: body.clientId, deletedAt: null },
				include: [
					{
						model: SubSegment,
						attributes: ['id'],
			...
			const dataItem = await this.generateDataItem(data, result, resultRotation);
			...
			const valueOfData = await this.generateValueOfData(body, data, dataItem, parsedFinalData, finalData);
	...
	async checkSegmentSubSegment(data: any, body: IImportLogCreate, user: User, transaction: Transaction) {
	...
	// set Value Of Data
	async generateValueOfData(body: IImportLogCreate, data: any, dataItem, parsedFinalData, finalData) {
		const valueOfData = {
			...
			weekendOvertimeBonus: this.extractNumber(dataItem, 'Weekend Overtime Bonus'),
	...
	extractCustomBonus(parsedFinalData: any, finalData: any) {
		return parsedFinalData && parsedFinalData.data.length > 0 ? finalData : null;
	}
	...
	async handleEmployee(isExist: any, dataItem, valueOfData, loginData, user, logId, transaction: Transaction) {
		...
		if (!isExist) {
			return await this.createEmployeeData(valueOfData, rollOver, loginData, user, transaction);
		} else {
	...
	async createEmployeeData(item: any, rollOver: string, loginData: any, user: User, transaction: Transaction = null) {
		try {
			...
			if (isExistMobileNumber) {
				throw new HttpException(400, 'Phone Number Already Exist in this platform', {}, true);
			}
	...
	async updateEmployeeData(
		...
		const isExistPhoneNumber = await LoginUser.findOne({
			where: { phone: loginData.phone, id: { [Op.ne]: isExistUser.loginUserId }, deletedAt: null },
			transaction,
		});
		...
		if (isExistUser?.rotationId != data.rotationId) {
			await this.handleEmployeeRotation(isExistUser, combineData, data, user, transaction);
		...
		if (isExistUser?.segmentId != data.segmentId || isExistUser?.subSegmentId !== data.subSegmentId) {
			await this.handleEmployeeSegment(isExistUser, combineData, data, user, transaction);
		}
		...
		await this.handleTimesheetOperations(isExistUser, empData, combineData, data, user, transaction);
		await this.importLogItems({
	...
	// set rotation data
	async handleEmployeeRotation(isExistUser: any, combineData, data: any, user: User, transaction: Transaction) {
	...
	// set segment data
	async handleEmployeeSegment(isExistUser: any, combineData, data: any, user: User, transaction: Transaction) {
	...
	async importLogItems({
		...
		if (errorItem !== 'error' && userId) {
			description = `Employee ${valueOfReturn ?? 'all'} updated using ${employeeNumber} employee number.`;
