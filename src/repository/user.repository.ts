...
import { BuildUserArgs, BuildUserUpdateArgs } from '../interfaces/functional/user.interface';
...
export default class UserRepo extends BaseRepository<User> {
	...
	readonly createUser = async (userCreateArgs: BuildUserArgs) => {
		...
		return db.transaction(async (transaction) => {
			const isExistUser: User & { isExistUser?: boolean } = await this.get({
				...
				include: [{ model: LoginUser, required: true, where: { email: data.email } }],
				attributes: ['id'],
				transaction,
			});
			if (!isExistUser) {
	...
	readonly updateUser = async (userCreateArgs: BuildUserUpdateArgs) => {
		...
		return db.transaction(async (transaction) => {
			...
			if (isExistUser) {
				...
				throw new HttpException(400, 'Email Address Not Registered.', null, true);
			}
		});
	};
	...
	readonly deleteUser = async (userCreateArgs: BuildUserUpdateArgs) => {
		...
		return db.transaction(async (transaction) => {
			...
			if (isExistUser) {
				const user = await this.update({ deletedAt: new Date() }, { where: { id: +isExistUser.id }, transaction });
				return user;
			}
		});
	};
	...
	readonly getUserRolePermissionData = async (loginUserId: number, roleId: number, clientId?: number) => {
	...
	async sendLink(data) {
		try {
			...
			const user = await User.findOne({
				include: [{ model: LoginUser, required: true, where: { email, deletedAt: null } }],
				rejectOnEmpty: false,
			});
