...
import { twilioMessages } from '@/helpers/twilioMessage.helper';
...
import { IMessageCreate, messageStatus } from '@/interfaces/model/message.interface';
...
export default class MessageRepo extends BaseRepository<Message> {
	...
	async getSalaryMessageEmployeeDataSuggestiveDropdown(query: IQueryParameters, transaction: Transaction = null) {
	...
	async getMessageByIdService(id: number) {
		let isFound = await Message.findOne({
			...
			attributes: ['id', 'message', 'errorMessage', 'status', 'isSchedule', 'scheduleDate', 'createdAt'],
			include: [
				{
					model: MessageDetail,
					attributes: ['employeeId', 'messageId', 'segmentId', 'managerUserId'],
	...
	async addMessage({ body, user }: { body: IMessageCreate; user: User }) {
		...
		const segmentEmpData = !_.isUndefined(insertSegmentData) && insertSegmentData.map((e) => e.segmentId);
		const managerUserData = !_.isUndefined(insertManagerUserData) && insertManagerUserData.map((e) => e.managerUserId);
		const empData = !_.isUndefined(insertEmpData) && insertEmpData.map((e) => e.employeeId);
	...
	async addSalaryMessage({ body, user }: { body: any; user: User }) {
		...
		const managerUser = body.messageSalary?.filter((e) => e.roleId === managerId);
			...
			insertEmpData = await Promise.all(
				body.messageSalary.map(async (e) => {
					...
					return {
						...
						monthlySalary: e?.monthlySalary ?? 0,
						bonusPrice: e.bonusPrice ?? 0,
						salaryDate: moment(moment(e?.salaryDate || null).format('DD-MM-YYYY'), 'DD-MM-YYYY').toDate(),
		...
		if (!_.isUndefined(managerUser)) {
			...
			insertManagerUserData = managerUser.map((e) => ({
				message:
					`LRED: Transfer of Janvier salary sent on ${moment(e?.salaryDate).format('DD/MM/YYYY')} with the bonus ${
						e?.bonusPrice ?? 0
					}. Salary total ${e?.total ?? 0}. Replies to this mobile aren't monitored.` + removeData,
				managerUserId: e.id,
				name: e.name,
				email: e.email,
				clientId: body.clientId,
		...
		let bulkCreateResult = await MessageSalary.bulkCreate([...(insertEmpData || []), ...(insertManagerUserData || [])]);
		bulkCreateResult = parse(bulkCreateResult);

		const id = !_.isUndefined(bulkCreateResult) && bulkCreateResult.map((e) => e.id);
		const managerUserData = !_.isUndefined(insertManagerUserData) && insertManagerUserData.map((e) => e.managerUserId);
		const empData = !_.isUndefined(insertEmpData) && insertEmpData.map((e) => e.employeeId);
	...
	async buildSalaryObject(value, user) {
		...
		const employeeData = {
			...
			firstName:
				value?.employeeDetail?.loginUserData?.firstName !== undefined
					...
					: value?.managerUser?.loginUserData?.firstName !== undefined && value?.managerUser?.loginUserData?.firstName,
			lastName:
				value?.employeeDetail?.loginUserData?.lastName !== undefined
					? value?.employeeDetail?.loginUserData?.lastName
					: value?.managerUser?.loginUserData?.lastName !== undefined && value?.managerUser?.loginUserData?.lastName,
			mobileNumber:
				value?.employeeDetail?.loginUserData?.phone !== undefined
					? value?.employeeDetail?.loginUserData?.phone
					: value?.managerUser?.loginUserData?.phone !== undefined && value?.managerUser?.loginUserData?.phone,
		...
		try {
			if (emailId) {
				await sendMail([emailId, 'admin@lred.com'], 'Email', 'employeeEmail', employeeData);
			}
	...
	async buildObject(item, messageData, body, user) {
		const emailId =
			item?.employeeDetail?.loginUserData?.email !== undefined
				...
				: item?.managerUser?.loginUserData?.email !== undefined && item?.managerUser?.loginUserData?.email;
		const employeeData = {
			email: emailId,
			firstName:
				item?.employeeDetail?.loginUserData?.firstName !== undefined
					? item?.employeeDetail?.loginUserData?.firstName
					: item?.managerUser?.loginUserData?.firstName !== undefined && item?.managerUser?.loginUserData?.firstName,
			lastName:
				item?.employeeDetail?.loginUserData?.lastName !== undefined
					? item?.employeeDetail?.loginUserData?.lastName
					: item?.managerUser?.loginUserData?.lastName !== undefined && item?.managerUser?.loginUserData?.lastName,
			mobileNumber:
				item?.employeeDetail?.loginUserData?.phone !== undefined
					? item?.employeeDetail?.loginUserData?.phone
					: item?.managerUser?.loginUserData?.phone !== undefined && item?.managerUser?.loginUserData?.phone,
			message: body.message,
			monthlySalary: '',
			bonusPrice: '',
			total: '',
	...
	async updateMessageService({ body, user, id }: { body: IMessageCreate; user: User; id: number }) {
		...
		const segmentEmpData = !_.isUndefined(insertSegmentData) && insertSegmentData.map((e) => e.segmentId);
		const managerEmpData = !_.isUndefined(insertManagerUserData) && insertManagerUserData.map((e) => e.managerUserId);
		const empData = !_.isUndefined(insertEmpData) && insertEmpData.map((e) => e.employeeId);
	...
	async deleteMessageService({ id }: { id: number }) {
