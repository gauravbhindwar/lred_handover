...
export default class ReliquatCalculationRepo extends BaseRepository<ReliquatCalculation> {
	constructor() {
		super(ReliquatCalculation.name);
	}

	async getAllReliquatCalculationService(req: Request, query: IQueryParameters) {
		let data;
		...
		if (_.isUndefined(req.query.listView)) {
			data = await this.getAllData({
				where: { ...(clientId !== undefined && { clientId: clientId }), deletedAt: null },
				include: [
					{
						model: Employee,
						attributes: ['id', 'loginUserId', 'contractEndDate', 'employeeNumber', 'slug'],
						as: 'employee',
						where: {
			...
			data = await this.getAll({
				...
				include: [
					{
						model: Employee,
						attributes: ['id', 'loginUserId', 'contractEndDate', 'employeeNumber'],
						as: 'employee',
						where: {
							// ...(employeeLoginData?.loginUserId && { loginUserId: employeeLoginData?.loginUserId }),
							// ...(isEmployeeLogin?.id === req?.user?.roleId && {
						...
						include: [
							{ model: EmployeeSegment, attributes: ['id', 'date', 'rollover', 'employeeId'] },
							{ model: LoginUser, attributes: ['firstName', 'lastName'] },
							{
								model: Rotation,
								attributes: ['name', 'id', 'weekOn', 'weekOff', 'description'],
		...
		try {
			...
			if (ReliquatCalculationLastRecord) {
				...
				if (timesheetData) {
					...
					const statusCountQuery = presenceOptions.map((dataItem) => `WHEN status = '${dataItem}' THEN 1`).join(' ');
					const statusCRCountQuery = leaveArray.map((dataItem) => `WHEN status = '${dataItem}' THEN 1`).join(' ');
					...
					const calculationdata = await TimesheetSchedule.findAndCountAll({
						...
						where: {
							...
							[Op.or]: [
								{
									status: { [Op.in]: presenceOptions.map((dataItem) => dataItem) },
								},
								{
									bonusCode: 'W',
								},
						...
						attributes: [
							[Sequelize.literal(`cast(SUM(CASE ${statusCountQuery} ELSE 0 END) as float)`), 'statusCount'],
							[Sequelize.literal(`cast(SUM(CASE ${statusCRCountQuery} ELSE 0 END) as float)`), 'statusCRCount'],
							// [
							// 	Sequelize.literal(`cast(SUM(CASE ${statusAbsenseCountQuery} ELSE 0 END) as float)`),
							// 	'statusAbsenseCount',
							// ],
							[Sequelize.literal(`cast(SUM(CASE ${statusBonusCountQuery} ELSE 0 END) as float)`), 'statusBonusCount'],
							[
								Sequelize.literal(
					...
					if (calculationdata?.rows?.length > 0) {
						...
						reliquatAdjustmentData?.map((adjustment) => {
							const isBetween = moment(adjustment.startDate).isBetween(timesheetData.startDate, date, null, '[]');
						...
						reliquatPaymentData?.map((payment) => {
							const isBetween = moment(payment.startDate).isBetween(timesheetData.startDate, date, null, '[]');
						...
						await temprotationData.reduce(
							(accumulator: any, currentValue, i, arr) => {
								...
								const countDatesInRange = dateListing.reduce((count, dateEntry) => {
									const entryDate = moment(dateEntry.date);

									if (entryDate.isSameOrAfter(currentValue.date) && entryDate.isSameOrBefore(accumulator)) {
										return count + 1;
									}
								...
								if (moment(startDate).isBefore(diffDate)) {
									tempData.count =
										moment(accumulator).add(1, 'days').diff(moment(diffDate), 'days') - countDatesInRange;
								} else {
									if (timesheetData.startDate === diffDate) {
										tempData.count =
											moment(accumulator).add(1, 'days').diff(moment(diffDate), 'days') - countDatesInRange;
									} else {
										tempData.count = moment(accumulator).diff(moment(diffDate), 'days') - countDatesInRange;
									}
									arr.splice(i);
								}
							...
							moment(employeeData.client.endDate).isBetween(timesheetData.startDate, timesheetData.endDate)
						...
						const tempRotationDayCount = await Promise.all(
							rotationDayCount.map(async (e) => {
								const rotation: any = await rotationData.find((e1) => e1?.rotationId == e?.rId);
								const temp = (rotation?.rotation.weekOff / rotation?.rotation.weekOn) * e.count;
						...
						const isWithinDateRange = employeeData?.employeeSegment.some((segment) => {
							const segmentDate = moment(segment.date);
							return segmentDate.isBetween(timesheetData.startDate, date, null, '[]');
						});
						...
						if (ReliquatCalculationLastRecord) {
							if (isWithinDateRange && !rolloverData) {
								...
								reliquatCalculationsData.totalWorked =
									+ReliquatCalculationLastRecord?.totalWorked + +timeSheetDataCount?.statusCount;
								reliquatCalculationsData.overtime =
									ReliquatCalculationLastRecord?.overtime + +timeSheetDataCount?.statusBonusCount || 0;
								reliquatCalculationsData.weekend = ReliquatCalculationLastRecord?.weekend + +sumOfWStatus || 0;
								reliquatCalculationsData.reliquatPayment =
									ReliquatCalculationLastRecord?.reliquatPayment + +paymentValue;
								reliquatCalculationsData.reliquatAdjustment =
									ReliquatCalculationLastRecord?.reliquatAdjustment + +adjustmentValue;
								reliquatCalculationsData.totalTakenLeave =
									ReliquatCalculationLastRecord.totalTakenLeave + +timeSheetDataCount.statusCRCount || 0;
								const lastRotationData = JSON.parse(
									ReliquatCalculationLastRecord.calculationDataJSON,
								).finalRotationCount;
								tempRotationDayCountForLoop = [...rotationDayCount, ...lastRotationData];
							}
						} else {
							reliquatCalculationsData.totalWorked = +timeSheetDataCount.statusCount;
							reliquatCalculationsData.overtime = +timeSheetDataCount?.statusBonusCount;
						...
						const finalRotationCount = tempRotationDayCountForLoop.reduce((accumulator, currentValue: any) => {
							const rotationExitData = accumulator.findIndex((e) => e.rId === currentValue.rId);
							if (rotationExitData > -1) {
								accumulator[rotationExitData].count += currentValue.count;
								accumulator[rotationExitData].rotationCalculation += currentValue.rotationCalculation;
							} else {
						...
						for (const item of finalRotationCount) {
							...
							const rotation: any = rotationData.find((e1) => e1.rotationId == item.rId);
							calculateEquation += `((${rotation?.rotation.weekOff || 0}/${rotation?.rotation.weekOn || 0}) * ${
								item.count || 0
						...
						earned +=
							...
							reliquatCalculationsData?.reliquatAdjustment -
							reliquatCalculationsData?.reliquatPayment;
						reliquatCalculationsData.earned = parseFloat(earned?.toFixed(2) || '0');
						reliquatCalculationsData.calculation = parseFloat(earned.toString() || '0');
						...
						reliquatCalculationsData.calculateEquation += `${
							reliquatCalculationsData?.reliquatPayment ? ` - ${reliquatCalculationsData?.reliquatPayment}` : ''
						}`;

						reliquatCalculationsData.reliquat = earned - reliquatCalculationsData.totalTakenLeave;
						reliquatCalculationsData.reliquat = parseFloat(reliquatCalculationsData?.reliquat || '0').toFixed(2);
						reliquatCalculationsData.reliquatValue = parseFloat(reliquatCalculationsData?.reliquat || '0').toFixed(2);
						reliquatCalculationsData.reliquat = Math.round(reliquatCalculationsData?.reliquat || '0');
						reliquatCalculationsData.calculationDataJSON = JSON.stringify(calculationDataJSON);
	...
	async addReliquatCalculationService(
		...
		try {
			...
			await this.timesheetData(employeeId, timesheetId, userId || null, transaction);
		} catch (error) {
			throw new Error(error);
		}
	}

	async timesheetData(empData: string, timesheetId: number, userId: number | null, transaction: Transaction = null) {
		try {
			const calculationDataJSON: any = {};
			...
			if (timesheetData) {
				...
				if (employeeData?.subSegmentId) {
					...
					// O1 = Number(employeeData?.subSegment?.overtime01Bonus);
					// O2 = Number(employeeData?.subSegment?.overtime02Bonus);
				} else if (employeeData?.segmentId && employeeData?.subSegmentId == null) {
					Wb1 = Number(employeeData?.segment?.fridayBonus);
					Wb2 = Number(employeeData?.segment?.saturdayBonus);
					// O1 = Number(employeeData?.segment?.overtime01Bonus);
					// O2 = Number(employeeData?.segment?.overtime02Bonus);
				...
				const statusCountQuery = presenceOptions.map((dataItem) => `WHEN status = '${dataItem}' THEN 1`).join(' ');
				const statusCACountQuery = caOptions.map((dataItem) => `WHEN status = '${dataItem}' THEN 1`).join(' ');
				const statusCRCountQuery = leaveArray.map((dataItem) => `WHEN status = '${dataItem}' THEN 1`).join(' ');
				...
				const calculationdata = await TimesheetSchedule.findAndCountAll({
					...
					where: {
						...
						[Op.or]: [
							...
							Sequelize.where(Sequelize.fn('concat', Sequelize.col('status'), ',', Sequelize.col('bonusCode')), {
								[Op.like]: '%W%',
							}),
							{
								status: { [Op.in]: leaveArray.map((dataItem) => dataItem) },
							...
							absenceFlag
								? {
										status: { [Op.in]: absenceArrOptions.map((dataItem) => dataItem) },
								  }
								: {},
							// {
							// 	status: { [Op.in]: absenceArray.map((dataItem) => dataItem) },
					...
					attributes: [
						[Sequelize.literal(`cast(SUM(CASE ${statusCountQuery} ELSE 0 END) as float)`), 'statusCount'],
						[Sequelize.literal(`cast(SUM(CASE ${statusCACountQuery} ELSE 0 END) as float)`), 'statusCACount'],
						[Sequelize.literal(`cast(SUM(CASE ${statusCRCountQuery} ELSE 0 END) as float)`), 'statusCRCount'],
						...(absenceFlag
							? [
									[
				...
				if (calculationdata?.rows?.length > 0) {
					...
					if (
						...
						rotationData?.find((findRotationData) => findRotationData?.rotation?.name !== 'Call Out')
					) {
						const rotationCalculationData = rotationData.find(
							(data) => data.rotationId === timeSheetDataCount.employee.rotationId,
						);
						if (
							...
							reliquatCalculationsData.totalWorked = +timeSheetDataCount.statusCount;
							reliquatCalculationsData.absenseDay = +timeSheetDataCount.statusAbsenseCount || 0;
							reliquatCalculationsData.annualLeave = +timeSheetDataCount.statusCACount || 0;
							reliquatCalculationsData.overtime = +timeSheetDataCount?.statusBonusCount;
							reliquatCalculationsData.weekend = +sumOfWStatus;
							reliquatCalculationsData.weekendBonus = +sumOfWStatus;
							reliquatCalculationsData.overtimeBonus = +timeSheetDataCount?.statusBonusCount;
							reliquatCalculationsData.totalTakenLeave =
								+timeSheetDataCount.statusCRCount + +timeSheetDataCount.statusCACount;
							reliquatCalculationsData.earned =
								(rotationCalculationData?.rotation.weekOff / rotationCalculationData?.rotation.weekOn) *
									(timeSheetDataCount.statusCount + timeSheetDataCount.statusAbsenseCount) +
								reliquatCalculationsData?.reliquatAdjustment -
								reliquatCalculationsData?.reliquatPayment;
							reliquatCalculationsData.reliquat = earned - timeSheetDataCount.statusCRCount;
							reliquatCalculationsData.reliquat = parseFloat(reliquatCalculationsData?.reliquat || '0').toFixed(2);
							reliquatCalculationsData.reliquatValue = parseFloat(reliquatCalculationsData?.reliquat || '0').toFixed(2);
							reliquatCalculationsData.reliquat = Math.round(reliquatCalculationsData?.reliquat || '0');
							reliquatCalculationsData.calculation = parseFloat(reliquatCalculationsData?.earned?.toString() || '0');
							const calculationString = `((${rotationCalculationData?.rotation.weekOff || 0}/${
								rotationCalculationData?.rotation.weekOn || 0
							}) * ${timeSheetDataCount.statusCount} = ${
								parseFloat(reliquatCalculationsData.earned?.toFixed(2) || '0') || 0
							}) +`;
							...
							calculationDataJSON.finalRotationCount = [{ ...calculateJSONData }];
							reliquatCalculationsData.calculationDataJSON = JSON.stringify(calculationDataJSON);
						} else if (
							timeSheetDataCount.statusCount === 0 &&
							(timeSheetDataCount.statusCRCount === 0 || timeSheetDataCount.statusCRCount !== 0) &&
							ReliquatCalculationLastRecord
						) {
							...
							reliquatCalculationsData.absenseDay = +ReliquatCalculationLastRecord?.absenseDay || 0;
							...
							reliquatCalculationsData.totalTakenLeave =
								...
								(timeSheetDataCount.statusCRCount !== 0
									? +timeSheetDataCount.statusCRCount + +timeSheetDataCount.statusCACount
									: 0);
							reliquatCalculationsData.earned = ReliquatCalculationLastRecord.earned;
							reliquatCalculationsData.reliquat =
								timeSheetDataCount.statusCRCount !== 0
									? ReliquatCalculationLastRecord?.reliquat - timeSheetDataCount.statusCRCount
							...
							reliquatCalculationsData.calculateEquation = ReliquatCalculationLastRecord?.calculateEquation;
							reliquatCalculationsData.calculationDataJSON = ReliquatCalculationLastRecord?.calculationDataJSON;
						} else {
							const rotationDayCount: any = [];

							const rolloverData =
								employeeData.employeeSegment.find((segment) => {
									return moment(segment.date).isBetween(momentStartDate, momentEndDate, null, '[]');
								})?.rollover ?? null;
							...
							const dateListing = combinedListing.filter((item) =>
								['', 'CR', ...absenceArray, ...(absenceFlag ? [] : absenceArrOptions)].includes(item.status),
							);
							const leaveListing = combinedListing.filter((item) => [...absenceArrOptions].includes(item.status));
							const temprotationData = [...rotationData];
							...
							await temprotationData.reduce(
								(accumulator: any, currentValue, i, arr) => {
									...
									if (absenceFlag) {
										const countAbsenceDatesInRange = leaveListing.filter((dateItem) => {
											const absenceDate = moment(dateItem.date);
											return absenceDate.isSameOrAfter(currentValue.date) && absenceDate.isSameOrBefore(accumulator);
										}).length;
									...
									const diffDate = moment(currentDate).isSameOrBefore(startDate) ? startDate : currentValue.date;
									...
									return moment(currentValue.date).isBetween(startDate, moment(timesheetData.endDate))
										? moment(currentValue.date).subtract(1, 'days')
										: moment(currentValue.date);
								},
								moment(employeeData.terminationDate).isBetween(timesheetData.startDate, timesheetData.endDate)
							...
							const tempRotationDayCount = await Promise.all(
								rotationDayCount.map((e) => {
									const rotation: any = rotationData.find((e1) => e1?.rotationId == e?.rId);
									const temp = (rotation?.rotation.weekOff / rotation?.rotation.weekOn) * e.count;
									e.rotationCalculation = temp;
									return { ...e };
								}),
							...
							if (ReliquatCalculationLastRecord) {
								if (!rolloverData && rolloverData !== null) {
									...
									reliquatCalculationsData.absenseDay =
										+ReliquatCalculationLastRecord?.absenseDay + +timeSheetDataCount?.statusAbsenseCount || 0;
									reliquatCalculationsData.overtime =
										ReliquatCalculationLastRecord?.overtime + +timeSheetDataCount?.statusBonusCount || 0;
									reliquatCalculationsData.weekend = ReliquatCalculationLastRecord?.weekend + +sumOfWStatus || 0;
							...
							const isBetweenStartAndEnd = currentDate.isBetween(momentStartDate, momentEndDate, null, '[]');

							if (isInCurrentMonth && isBetweenStartAndEnd && reliquatCalculationsData.reliquat < 0) {
								const emails = [];
								...
								const context = {
									...
									checkReliquatUrl:
										employeeData?.id && employeeData?.clientId
											? FRONTEND_URL + `/employee/reliquat-calculation/${employeeData?.id}_${employeeData?.clientId}`
											: '',
								};
								await sendMail(emails, 'Low Reliquat', 'lowReliquat', context);
								// }
							}
						}
						await ReliquatCalculation.create(reliquatCalculationsData, { transaction });
					}
				}
			}
		} catch (error) {
