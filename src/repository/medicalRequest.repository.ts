...
import { IMedicalRequestCreate, medicalRequestStatus } from '@/interfaces/model/medicalRequest.interface';
...
export default class MedicalRequestRepo extends BaseRepository<MedicalRequest> {
	...
	async getAllMedicalRequests(query: IQueryParameters) {
		const { page, limit, sortBy, sort, clientId, employeeId, startDate, endDate } = query;
		const sortedColumn = sortBy || null;
		// End Date Convert with timestamp with timezone
		let dateWithTimezone = null;
		if (startDate && endDate) {
			dateWithTimezone = new Date(new Date(endDate).getTime() - new Date(endDate).getTimezoneOffset() * 60000);
		...
		let data = await this.getAllData({
			where: {
				...
				...(startDate && endDate && dateWithTimezone
					? {
							createdAt: {
								[Op.between]: [startDate, new Date(dateWithTimezone).toISOString()],
							},
					  }
					: {}),
				deletedAt: null,
	...
	async addMedicalRequest({ body, user }: { body: IMedicalRequestCreate; user: User }) {
		const lastData = await MedicalRequest.findOne({ order: [['createdAt', 'DESC']] });
		const reference = `LRED/MC/${moment().format('DDMMMYY')}/${String(lastData ? lastData.id + 1 : 1).padStart(
			4,
			'0',
		)}`.toUpperCase();
		const medicalDate = moment(moment(body.medicalDate).format('DD/MM/YYYY'), 'DD/MM/YYYY').toDate();

		let medicalTypeData = await MedicalType.findOne({
			where: {
				id: body.medicalTypeId,
			},
			attributes: ['daysExpiry', 'format', 'name', 'daysBeforeExpiry', 'amount'],
		});
		medicalTypeData = parse(medicalTypeData);
		const medicalExpiry = moment(moment(body.medicalDate).format('DD/MM/YYYY'), 'DD/MM/YYYY')
			.add(medicalTypeData.daysExpiry, 'days')
			.toDate();

		const formatType = formatOptionData?.find((format) => format.title === medicalTypeData.format);

		const employeeData = await Employee.findOne({
			where: { id: body.employeeId, deletedAt: null },
			include: [
				{ model: LoginUser, attributes: ['id', 'email', 'firstName', 'lastName'] },
				{
					model: Client,
		...
		if (formatType?.value === 1) {
			const medicalRequest = await this.get({
				where: { employeeId: body.employeeId, medicalTypeId: body.medicalTypeId, deletedAt: null, status: 'ACTIVE' },
				include: { model: MedicalType, where: { format: formatType.title } },
			});
			if (medicalRequest) {
				throw new HttpException(400, `${medicalTypeData.name} medical type request already exist`, null, true);
			} else if (
				!medicalRequestDate.isBetween(moment(employeeData.startDate), moment(employeeData.startDate).add(2, 'months'))
			)
				throw new HttpException(400, `${medicalTypeData.name} medical type request has been expired`, null, true);
		} else if (formatType?.value === 2 && employeeData.medicalCheckExpiry) {
			if (
				!medicalRequestDate.isBefore(
					moment(employeeData.medicalCheckExpiry).subtract(medicalTypeData.daysBeforeExpiry ?? 0, 'days'),
				)
			)
				throw new HttpException(
					400,
					`${medicalTypeData.name} medical type request has been allow only before ${
		...
		if (data) {
			...
			let emails = [];
			if (employeeData?.client?.medicalEmailSubmission || employeeData?.client?.medicalEmailSubmission !== '') {
			...
			if (medicalType) {
				const replacement = {
					...
					checkReliquatUrl: '',
					message: `The medical check for ${medicalType?.name} for ${moment(data?.medicalDate).format(
						'DD MMMM YYYY',
					)} for the respective employee has been created successfully`,
				};
				if (employeeData?.loginUserData?.email && !emails.includes(employeeData?.loginUserData?.email)) {
				...
				if (medicalRequestData) {
					...
					const footerContent = `Submitted by ${medicalRequestData.createdByUser.loginUserData.name} on ${moment(
						medicalRequestData.createdAt,
					).format('DD MMMM YYYY')} at ${moment(medicalRequestData.createdAt).format('LT')}`;
					...
					const pdfReplacement = {
						...
						medicalDate: moment(medicalRequestData?.medicalDate).format('DD MMMM YYYY'),
				...
				if (emails && emails.length > 0) {
					await sendMail(emails, 'Medical Check Details', 'generalMailTemplate', replacement, [{ path: filePath }]);
				}
			}
		}
	...
	async updateMedicalRequestStatus({ user, id }: { user: User; id: number }) {
		...
		if (isExistMedicalRequest.medicalTypeData.name) {
			const replacement = {
				...
				mailHeader: `Medical Check Details`,
				checkReliquatUrl: '',
				message: `The medical check for ${isExistMedicalRequest.medicalTypeData?.name} for ${moment(
					isExistMedicalRequest?.medicalDate,
				).format('DD MMMM YYYY')} for the respective employee has been cancelled successfully`,
			};
			if (employeeData?.loginUserData?.email && !emails.includes(employeeData?.loginUserData?.email)) {
				emails.push(employeeData?.loginUserData?.email);
			}
			...
			const medicalRequestData = await this.getMedicalRequestById(isExistMedicalRequest.id);
			const medicalTypeListData = await this.MedicalTypeService.getMedicalTypesData();
			const pdfName = `${moment().unix()}-medical-request.pdf`;
			if (medicalRequestData) {
				const resizeHeaderFooter = false;
				const footerContent = `Submitted by ${medicalRequestData.createdByUser.loginUserData.name} on ${moment(
					medicalRequestData.createdAt,
				).format('DD MMMM YYYY')} at ${moment(medicalRequestData.createdAt).format('LT')}, cancelled by ${
					medicalRequestData?.updatedByUser.loginUserData.name
				} on ${moment(medicalRequestData.updatedAt).format('DD MMMM YYYY')} at ${moment(
					medicalRequestData.updatedAt,
				).format('LT')}`;
				let typeList = `<td style="border: 1px solid #000; padding: 10px 0 0;">`;
				medicalTypeListData?.data?.map((item) => {
					if (item.id === medicalRequestData?.medicalTypeId) {
