...
import { ITransportRequestCreate, TransportRequestAttributes } from '@/interfaces/model/transport.request.interface';
...
export default class TransportRequestRepo extends BaseRepository<TransportRequest> {
	constructor() {
					...
					include: [
						{
							model: LoginUser,
							attributes: ['email'],
							where: { ...(search && { email: { [Op.iLike]: '%' + search.toLowerCase() + '%' } }) },
						},
					],
	...
	async addTransportRequest({ body, user }: { body: ITransportRequestCreate; user: User }) {
	...
	async updateTransportRequest({ body, user, id }: { body: TransportRequestAttributes; user: User; id: number }) {
		...
		await TransportRequest.update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteTransportRequest(id: number) {
		let isExistTransportRequest = await TransportRequestVehicle.findAll({
			...
			include: [
				{ model: TransportRequest, attributes: ['id', 'startDate', 'destinationDate'] },
		...
		if (isExistTransportRequest) {
			for (const element of isExistTransportRequest) {
				...
				endingDate = moment(element.request.destinationDate).format('DD/MM/YYYY');
