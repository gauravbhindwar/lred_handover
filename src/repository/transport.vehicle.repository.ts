...
import { ITransportVehicleCreate, TransportVehicleAttributes } from '@/interfaces/model/transport.vehicle.interface';
...
export default class TransportVehicleRepo extends BaseRepository<TransportVehicle> {
	constructor() {
	...
	async getAllTransportVehicle(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			where: {
				...
				...(search && { vehicleNo: { [Op.iLike]: '%' + search.toLowerCase() + '%' } }),
			},
	...
	async addTransportVehicle({ body, user }: { body: ITransportVehicleCreate; user: User }) {
		...
		let data = await TransportVehicle.create({ ...filterData, capacity, createdBy: user.id });
		data = parse(data);
	...
	async updateTransportVehicle({ body, user, id }: { body: TransportVehicleAttributes; user: User; id: number }) {
		const isExistTransportVehicle = await TransportVehicle.findOne({
			where: {
				id: id,
				deletedAt: null,
