...
export default class UserClientRepo extends BaseRepository<UserClient> {
	...
	async updateUserClientData({
		...
		for (const element of body.client) {
			const isExist = await UserClient.findOne({ where: { clientId: element, userId: id, deletedAt: null } });
			if (!isExist) {
				...
				UserPermission.destroy({
					where: { loginUserId: isExistUser?.loginUserId, clientId: Number(element) },
					force: true,
				});
				const rolePermissions = await RolePermission.findAll({ where: { roleId: isExistUser?.roleId } });
		...
		const userClientList = await UserClient.findAll({ where: { userId: id, clientId: { [Op.notIn]: body.client } } });
		userClientList?.map(async (userClient) => {
			const segmentData = await Segment.findAll({ where: { clientId: userClient.clientId } });
			const segmentIds = segmentData?.map((segment) => segment.id);
			if (segmentIds.length) {
				await UserSegment.destroy({
					where: {
						userId: userClient.userId,
						segmentId: { [Op.in]: segmentIds, ...(clientId ? { clientId: clientId } : {}) },
					},
				});
		...
		await UserClient.destroy({ where: { userId: id, clientId: { [Op.notIn]: body.client } } });
	...
	readonly deleteUserClient = async (id: number, clientId?: number) => {
		const isExistUserClient = await this.get({
			where: { id, deletedAt: null },
		});
		if (isExistUserClient) {
			await this.deleteData({ where: { id: id } });
			const segmentData = await Segment.findAll({ where: { clientId: isExistUserClient.clientId } });
			const segmentIds = segmentData?.map((segment) => segment.id);
			if (segmentIds.length) {
				await UserSegment.destroy({
					where: {
