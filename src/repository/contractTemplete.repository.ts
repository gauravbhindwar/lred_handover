...
export default class ContractTemplateRepo extends BaseRepository<Segment> {
	...
	async addContractTemplate({ body, user }: { body: IContractTemplateCreate; user: User }) {
	...
	async updateContractTemplateStatus({ body, id }: { body: IContractTemplateCreate; id: number }) {
		const isExistClient = await ContractTemplate.findOne({ where: { id: id } });
		if (!isExistClient) {
			throw new HttpException(404, this.msg.notFound);
		}

		await ContractTemplate.update({ isActive: body.isActive }, { where: { id: id } });
		const data = await this.getContractTemplateByIdService(id);
		return data;
	}

	async updateContractTemplate({ body, user, id }: { body: IContractTemplateCreate; user: User; id: number }) {
		const isExist = await ContractTemplate.findOne({ where: { id: id, deletedAt: null } });
		if (!isExist) {
			throw new HttpException(200, this.msg.notFound, {}, true);
		}
		await ContractTemplate.update({ ...body, updatedBy: user.id }, { where: { id: id } });
		const data = await this.getContractTemplateByIdService(id);
	...
	async deleteContractTemplateService({ id }: { id: number }) {
