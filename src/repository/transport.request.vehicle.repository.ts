...
export default class TransportRequestVehicleRepo extends BaseRepository<TransportRequestVehicle> {
	constructor() {
	...
	async getAllTransportRequestVehicle(query: IQueryParameters) {
		...
		let data = await this.getAllData({
			where: { deletedAt: null, requestId: requestId, ...(clientId && { clientId: clientId }) },
	...
	async getTransportRequestVehicleById(id: number) {
		let data = await TransportRequestVehicle.findOne({
			...
			include: [
				{ model: TransportVehicle, attributes: ['vehicleNo'], where: { deletedAt: null } },
				{ model: TransportDriver, attributes: ['driverNo'], where: { deletedAt: null } },
			],
		});
		if (!data) {
	...
	async addTransportRequestVehicle({
		...
		let data = await TransportRequestVehicle.create({ ...body, createdBy: user.id });
		if (data) {
	...
	async updateTransportRequestVehicle({
		...
		await TransportRequestVehicle.update({ ...body, updatedBy: user.id }, { where: { id: id, deletedAt: null } });
	...
	async deleteTransportRequestVehicle(id: number, query: IQueryParameters) {
		const unavailableDates = query.transportStartDate + '-' + query.transportEndDate;
...
const updateOldData = async (data, unavailableDates) => {
	...
	if (!_.isEmpty(oldVehicleData.unavailableDates)) {
		finalVehicleUnavailableDate = oldVehicleData.unavailableDates.concat(',', unavailableDates);
	}
	if (!_.isEmpty(oldDriverData.unavailableDates)) {
		finalDriverUnavailableDate = oldDriverData.unavailableDates.concat(',', unavailableDates);
	}

	await TransportVehicle.update({ unavailableDates: finalVehicleUnavailableDate }, { where: { id: data.vehicleId } });
	await TransportDriver.update({ unavailableDates: finalDriverUnavailableDate }, { where: { id: data.driverId } });
};
