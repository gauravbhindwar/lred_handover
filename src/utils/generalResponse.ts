...
const generalResponse = (
	...
	if (_.isArray(data) || _.isObject(data)) data = sanitizeDateValuesInObject(data);
	if (statusCode !== 200) {
...
export function sanitizeDateValuesInObject(result: any): any {
	if (_.isArray(result)) return result.map((r) => sanitizeDateValuesInObject(r));
	if (!_.isObject(result)) return result;
	result = flatten(result);
	result = _.mapValues(result, (value, _key) => {
		if (value instanceof Date && value.toISOString().endsWith('T00:00:00.000Z')) {
			return value;
		}
