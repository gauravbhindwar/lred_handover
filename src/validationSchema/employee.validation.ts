...
export const EmployeeCreateSchema = Joi.object({
	...
	isAbsenseValueInReliquat: Joi.boolean().label('Is Absense Value In Reliquat').allow('', null).default(false),
	...
	dOB: [Joi.date().label('Date of Birth').allow('', null), Joi.string().label('Date of Birth').allow('', null)],
	...
	LREDContractEndDate: Joi.date().label('LRED Contract End Date').allow('', null),
	catalogueNumber: [
	...
	photoVersionNumber: Joi.number().label('Photo Version Number').allow('', null),
	...
	weekendOvertimeBonus: Joi.number().label('Weekend Overtime Bonus').allow('', null),
	...
	employeeStatus: joiCommon.joiString.valid('DRAFT', 'SAVED').default(employeeStatus.SAVED),
}).options({
	abortEarly: false,
});
