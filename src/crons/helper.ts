	...
	for (const tempTimeSheetData of autoUpdateEndDateClientList) {
		...
		try {
			...
			await Client.update(
				...
				{ where: { id: tempTimeSheetData?.id }, transaction },
			);
...
const checkDate = async (item, table) => {
	...
	if (table === 'transportRequest') {
		...
		if (destinationDate.isSameOrAfter(currDate) && startDate.isSameOrBefore(currDate)) {
			...
			await TransportRequest.update({ status: transportStatus.COMPLETED }, { where: { id: item.id, deletedAt: null } });
		} else {
			await TransportRequest.update({ status: transportStatus.STARTED }, { where: { id: item.id, deletedAt: null } });
		}
	}
};
