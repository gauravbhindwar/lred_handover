...
import { fileDelete, folderExistCheck, generateFourDigitNumber, parse } from '@/utils/common.util';
...
class TimesheetController {
	...
	private setNewScheduleValue = (employeeDate, employeeData, userId, rotationDetails, clientData) => {
		let employeeMonthSchedule = [];
		if (
			moment(employeeData.startDate).isSameOrBefore(employeeDate) &&
			(employeeData.terminationDate == null || moment(employeeData.terminationDate).isSameOrAfter(employeeDate)) &&
			(clientData.autoUpdateEndDate != 0 ||
				(clientData.autoUpdateEndDate == 0 && moment(employeeDate).isSameOrBefore(clientData.endDate)))
		) {
			if (!rotationDetails?.weekOn || !rotationDetails?.weekOff) {
				if (
					employeeData.timeSheetSchedule.findIndex(
						(dat) => dat.dbKey == `${employeeDate.format('DDMMYYYY')}${employeeData.id}`,
					) < 0
				) {
				...
				if (this.empRotationDetail.activeSegment == 1) {
					...
					if (this.empRotationDetail.activeCount == this.empRotationDetail.totalWorkingDays) {
					...
					if (
						this.empRotationDetail.activeCount == this.empRotationDetail.totalOffDays ||
						this.empRotationDetail.totalOffDays == 0
					) {
						if (
							this.empRotationDetail.totalOffDays == 0 &&
			...
			if (
				(employeeData.terminationDate != null && moment(employeeData.terminationDate).isBefore(employeeDate)) ||
				moment(employeeDate).isBefore(employeeData.startDate) ||
				(clientData.autoUpdateEndDate == 0 && moment(employeeDate).isAfter(clientData.endDate))
			) {
	...
	private getObjectFromNestedArray = (arr) => {
		for (const item of arr) {
			if (Array.isArray(item)) {
				...
				this.allEmployeeTimesheetSchedule = [...this.allEmployeeTimesheetSchedule, { ...item }];
			}
		}
		return null;
	};
	...
	private extractSubSegment = (arr) => {
		for (const item of arr) {
			...
			} else if (typeof item === 'object' && !this.allSubSegment.find((a) => a.id == item.id)) {
		...
		await Promise.all(
			dataArray?.map(async (data) => {
				if (
					...
					(data.terminationDate == null || moment(data.terminationDate).add(1, 'month').isAfter(endDate.toDate()))
				) {
					...
					if (
						...
						for (const segmentData in findEmployeeSegment) {
							if (!moment(startDate).isSame(findEmployeeSegment[Number(segmentData)]?.date)) {
								let newStartDate = startDate;
								let newEndDate = endDate;
								data.segmentId = findEmployeeSegment[Number(segmentData)]?.segmentId ?? null;
								data.subSegmentId = findEmployeeSegment[Number(segmentData)]?.subSegmentId ?? null;
								if (findEmployeeSegment?.length > 1) {
									if (+segmentData === 0) {
										newEndDate = moment(
											moment(findEmployeeSegment[Number(segmentData)]?.date).format('DD-MM-YYYY'),
											'DD-MM-YYYY',
										).subtract(1, 'days');
										continue;
										...
										await Timesheet.update(
											...
											},
											{
												where: {
													startDate: moment(
														moment(findEmployeeSegment[Number(segmentData) - 1]?.date).format('DD-MM-YYYY'),
														'DD-MM-YYYY',
													),
													employeeId: data?.id,
												},
								...
								const body = {
									...
									dbKey: `${moment(newStartDate).format('DDMMYYYY')}${data.segmentId}${data?.subSegmentId || ''}${
										data.id
									}`,
								};
							...
							} else if (moment(startDate).isSame(findEmployeeSegment[Number(segmentData)]?.date)) {
								await this.TimesheetService.update(
									{
										segmentId: findEmployeeSegment[Number(segmentData)]?.segmentId,
										subSegmentId: findEmployeeSegment[Number(segmentData)]?.subSegmentId ?? null,
										updatedBy: user?.id,
										dbKey: `${moment(startDate).format('DDMMYYYY')}${data.segmentId}${data?.subSegmentId || ''}${
											data.id
										}`,
									},
									{
						...
						const body = {
							...
							employeeId: data.id,
							// dbKey: `${startDate.format('DDMMYYYY')}${data.id}`,
							dbKey: `${moment(startDate).format('DDMMYYYY')}${data.segmentId}${data?.subSegmentId || ''}${data.id}`,
						};
						const isExistTimesheet = await this.TimesheetService.get({
							where: {
								employeeId: body.employeeId,
						...
						if (!isExistTimesheet) {
							...
							responseData = parse(responseData);
							return responseData;
						} else if (!isTimesheetApprovedData?.some((e) => e.status === 'APPROVED')) {
							if (isSegmentAdd && moment(employeeSegmentData?.date).isSameOrAfter(startDate)) {
								body.segmentId = isExistTimesheet.segmentId;
								body.subSegmentId = isExistTimesheet?.subSegmentId ?? null;
							}
							await this.TimesheetService.update(
								{
									...body,
					...
					await this.TimesheetService.deleteTimesheetSummaryService({ ...body }, transaction);
				}
			}),
		);
	};

	public getAllTimesheetData = catchAsync(async (req: Request, res: Response) => {
		const responseData = await this.TimesheetService.getAllTimesheetService(req.query, req.user as User);
	...
	public constructTimesheetDate = async (ClientData, isExtendDate: boolean) => {
		...
		startDate = moment([startDate.year(), startDate.month(), ClientData.clientTimesheetStartDay[0].timesheetStartDay]);
		const endDate = moment(startDate).add(1, 'month').subtract(1, 'day');

		let ClientEndDate: Date = null;
		if (moment(ClientData.endDate).endOf('month').isBefore(moment().startOf('month')) && ClientData.autoUpdateEndDate) {
		...
		if (ClientData.autoUpdateEndDate && isExtendDate) {
			difference = Math.ceil(moment(ClientEndDate).diff(startDate, 'months', true)) + ClientData.autoUpdateEndDate;
		} else {
			difference = Math.ceil(moment(ClientEndDate).diff(startDate, 'months', true));
		}
	...
	public getTimesheetEmployeeDetails = async (clientId, employeeIds = [], transaction: Transaction = null) => {
		const condition = employeeIds.length > 0 ? { id: { [Op.in]: employeeIds } } : { clientId: clientId };
	...
	public generateTimesheetSummary = async (
		...
		for (let i = 0; i < Math.round(difference); i++) {
			if (
				...
				moment(ClientData.clientTimesheetStartDay[activeIndex].date).isBetween(newStartDate, newEndDate, null, '[]')
			) {
				const diff =
	...
	public generateTimesheetSchedule = async (
		...
		return Promise.all(
			allEmployees?.map(async (employeeData) => {
				...
				let empResp = [];
				startDate = moment(ClientData.startDate);
				startDate = moment([startDate.year(), startDate.month(), ClientData.timeSheetStartDay]);
				const rotationDetailsLength: number = employeeData.employeeRotation.length;
				let newStartDate = startDate;
				let k: string;
				if (employeeData.employeeRotation?.length > 0) {
					for (k in employeeData.employeeRotation) {
						...
						if (k == String(rotationDetailsLength - 1)) {
							endDate = moment(startDate).add(difference, 'month').subtract(1, 'day');
						} else {
							endDate = moment(employeeData.employeeRotation[Number(k) + 1].date);
						}
						...
						for (let j = 0; j < endOfMonth; j++) {
							empResp =
								this.setNewScheduleValue(employeeDate, employeeData, user?.id || null, rotationDetails, ClientData) ??
								null;
							if (empResp) {
					...
					employeeDate = moment(newStartDate);
					for (let j = 0; j < endOfMonth; j++) {
						empResp = this.setNewScheduleValue(employeeDate, employeeData, user?.id || null, null, ClientData) ?? null;
						if (empResp) {
							employeeMonthlyDetails = [...employeeMonthlyDetails, ...empResp];
						}
						employeeDate = employeeDate.add(1, 'days');
				...
				empResp =
					this.setNewScheduleValue(
						...
						employeeData,
						user?.id || null,
						employeeData.employeeRotation[employeeData.employeeRotation.length - 1]?.rotation,
						ClientData,
					) ?? null;
				if (empResp) {
					employeeMonthlyDetails = [...employeeMonthlyDetails, ...empResp];
	...
	public createTimesheet = async (
		...
		try {
			const ClientData = await this.getClientForTimesheet(clientId, transaction);
			const constructedTimesheetDates = await this.constructTimesheetDate(ClientData, isExtendDate);
			...
			const allEmployees = await this.getTimesheetEmployeeDetails(clientId, employeeIds, transaction);

			if (startDate && endDate) {
				let employeeDate;

				if (!disableFunction.includes('timesheetSummary')) {
					await generateModalData({ user: user, percentage: 20, message: 'Generating Timesheet' });
				...
				if (!disableFunction.includes('timesheetSchedule')) {
					await generateModalData({ user: user, percentage: 40, message: 'Generating Timesheet' });
						...
						where: {
							...
							date: {
								[Op.and]: {
									...
									[Op.gt]: moment(lastTimesheetApproved?.endDate).format('YYYY-MM-DD'),
								},
							},
						},
						transaction,
				...
				if (!disableFunction.includes('reliquet')) {
					await generateModalData({ user: user, percentage: 70, message: 'Generating Reliquat Calculation' });
					const condition = employeeIds?.length > 0 ? employeeIds : null;
					await this.TimesheetService.generateReliquetResponse(user, condition, transaction, clientId);
				}
				// await generateModalData({ user: user, percentage: 90, message: 'Generating Accounts' });
				const condition = employeeIds?.length > 0 ? employeeIds : null;
				await this.TimesheetService.generateAccountData(user, condition, type, clientId, transaction);
			}
		} catch (error) {
			console.log({ error });
	...
	public createTimesheetApi = catchAsync(async (req: Request, res: Response) => {
		...
		const employeeIds = req.body.employeeIds?.length > 0 ? req.body.employeeIds : [];
			...
			const promise = new Promise(async (resolve) => {
				try {
					await this.createTimesheet(
						{ clientId: cid, user, employeeIds, disableFunction: disabledFunctionality },
						transaction,
					);
					resolve(cid);
					transaction.commit();
		...
		return generalResponse(req, res, promises, this.msg.create, 'success', false);
	});
	 ...
	 */
	public getDropdownDetails = catchAsync(async (req: Request, res: Response) => {
		const clientId = req.params.clientId;
	...
	public approveTimesheet = catchAsync(async (req: Request, res: Response) => {
		...
		try {
			...
			const actualStartDate = moment(req.body.startDate, 'DD-MM-YYYY').toDate();
			const actualEndDate = moment(req.body.endDate, 'DD-MM-YYYY').toDate();
			...
			for (const timesheetId of timesheetIds) {
				const promise = new Promise(async (resolve) => {
					const timesheetLogsData = await TimesheetLogs.create(
						{
							...
							status: newStatus === 'APPROVED' ? timesheetLogsStatus.APPROVED : timesheetLogsStatus.UNAPPROVED,
							actionDate: new Date(),
						},
						{ transaction },
					).then((parseData) => parse(parseData));
					const response = await this.TimesheetService.approveTimesheet(
						{ timesheetId, newStatus, timesheetLogsData, user: req.user as User, actualStartDate, actualEndDate },
						transaction,
					);
					...
					if (newStatus === 'UNAPPROVED') {
						const timesheetScheduleData = await this.TimesheetService.findTimesheetSummaryById(result.id, transaction);
						timesheetScheduleData && timesheetApprovedList.push(parse(timesheetScheduleData));
					}
					if (newStatus === 'APPROVED') {
						await generateModalData({ user: req.user as User, percentage: 75, message: 'Updating Accounts' });
					...
					// if (newStatus == 'APPROVED') {
					// 	await this.TimesheetService.generateReliquetResponse(req.user, [result?.employeeId], transaction, null, []);
					// }
					resolve(result);
				});
			...
			if (newStatus === 'APPROVED') {
				...
				this.sendSegmentTimesheetMail(timesheetIds, newStatus, req, actualStartDate, actualEndDate);
			} else {
				this.sendUnapprovalMail(timesheetApprovedList);
			}
			return generalResponse(req, res, resp, `Timesheet ${newStatus} !!`, 'success', true);
		} catch (error) {
			console.log({ error });
			await transaction.rollback();
			return generalResponse(req, res, error, `Something went Wrong !!} `, 'failed', true, 400);
		}
	});
			...
			for (const mailData of sendMailData) {
				try {
					...
					if (mailData?.emails?.length > 0) {
						...
						await sendMail(mailData?.emails, 'Timesheet Approval Details', 'generalMailTemplateTable', replacement, [
							{ path: pdfPath },
						]);
					} else {
						fileDelete(pdfPath);
	...
	async sendUnapprovalMail(timesheetApprovedList) {
		if (timesheetApprovedList?.length > 0) {
			...
			const replacement = {
				...
				client: timesheetApprovedList[0]?.employeeData?.client?.loginUserData?.name,
				timesheetPeriod:
					timesheetApprovedList[0].timesheetData.startDate + '-' + timesheetApprovedList[0].timesheetData.endDate,
			...
			await sendMail(['admin@lred.com'], 'Timesheet Unapproved', 'unapproveTimesheet', replacement);
		}
	}
		...
		try {
			...
			if (timesheetApprovedList.length && newStatus === 'APPROVED') {
				...
				for (const timesheetApprovedData of timesheetApprovedList) {
					const segmentName = `${timesheetApprovedData?.timesheetData?.segment?.name}${
					...
					if (!isExist) {
						segmentArr.set(segmentName, {
							...
							subSegment: timesheetApprovedData?.timesheetData?.subSegment?.name ?? null,
						});
					}
				}
				...
				for (const segmentData of finalSegmentArr) {
					const filteredData = timesheetApprovedList?.filter(
						(e) =>
							(segmentData?.segment &&
								...
								(segmentData.subSegment ? e?.timesheetData?.subSegment?.name === segmentData?.subSegment : '')) ||
							(segmentData?.segment && e?.timesheetData?.segment?.name === segmentData?.segment),
					);
					...
					filteredData?.map((items) => {
						...
						items?.overtimeWeekendBonus?.map((e: { label: string; title?: string; length: number }) => {
							if (weekendOvertimeColsArr?.findIndex((isExist) => isExist === e?.label) < 0) {
								weekendOvertimeColsArr?.push(e?.label);
								weekendOvertimeData?.push({ label: e.label, title: e?.title ?? '' });
							}
						});

						if (items?.employeeData?.reliquatCalculation && items?.employeeData?.reliquatCalculation?.length > 0)
							isReliquat = true;

						if (items?.employeeData?.rotation && items?.employeeData?.rotation?.name != 'Call Out') {
							isReliquatTotal = true;
						}
						pdfEmployeeData.push({
							employeeName:
								items?.employeeData?.loginUserData?.lastName + ' ' + items?.employeeData?.loginUserData?.firstName,
							employeeNumber: items?.employeeData?.employeeNumber,
							status: items?.status,
							statusCounts: items?.statusCounts,
						});
					});
					const isHourlyBonusNote = filteredData?.some((e) => e?.isHourlyBonusNote);
					const pdfData = {
					...
					const stampLogo =
						filteredData && filteredData.length > 0 && filteredData[0]?.employeeData?.client?.stampLogo
					...
					const itemData = filteredData[0].timesheetData.timesheetLogsData?.map((item, index) => {
					...
					if (
						...
						emails = emails.concat(filteredData[0].employeeData.client.approvalEmail.split(','));
					...
					const replacement = {
						...
						timesheetPeriod: filteredData[0].timesheetData.startDate + '-' + filteredData[0].timesheetData.endDate,
					...
					const filePath = path.join(publicFolder, `segmentTimesheetPdf/${pdfName}`);
					if (emails.length > 0) {
						await sendMail(emails, 'Segment Timesheet Approval Details', 'approveTimesheet', replacement, [
							{ path: filePath },
						]);
					} else {
						fileDelete(filePath);
	...
	public getReliquatAdjustmentDates = catchAsync(async (req: Request, res: Response) => {
		const clientId = req.params.clientId;
		...
		return generalResponse(req, res, resp, 'Reliquat Adjustment Date Fetched Successfully', 'success', false);
	});

	public findTimesheetSummaryById = catchAsync(async (req: Request, res: Response) => {
		const ids = req.query?.id ? req.query?.id.toString().split(',') : [];
		const startDate = moment(req?.query?.startDate.toString(), 'DD-MM-YYYY').toDate();
		const endDate = moment(req?.query?.endDate.toString(), 'DD-MM-YYYY').toDate();
		const responseDatas = [];
		if (ids.length) {
			for (const idData of ids) {
				const result = await this.TimesheetService.findTimesheetSummaryById(+idData, null, startDate, endDate);
				result && responseDatas.push(result);
			}
		}
		return generalResponse(req, res, responseDatas, this.msg.fetch, 'success', false);
	});
}
