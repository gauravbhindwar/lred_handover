...
class BonusTypeController {
	...
	public findAllBonusType = catchAsync(async (req: Request, res: Response) => {
		const responseData = await this.BonusTypeService.getAllBonusTypeService(req.query);
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});

	public getBonusTypeDropdownData = catchAsync(async (req: Request, res: Response) => {
	...
	public findBonusTypeById = catchAsync(async (req: Request, res: Response) => {
		...
		const responseData = await this.BonusTypeService.getBonusTypeByIdService(Number(id));
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});
	 ...
	 * @returns {Promise<void>}
	 */
	public updateBonusTypeStatus = catchAsync(async (req: Request, res: Response) => {
		const id = req.params.id;
		...
		return generalResponse(
			...
			res,
			responseData,
			responseData.isActive === true ? 'Bonus Type Activated Successfully' : 'Bonus Type Archived Successfully',
			'success',
			true,
		);
	});
