...
class EmployeeLeaveController {
	...
	public findAllEmployeeLeaveTypes = catchAsync(async (req: Request, res: Response) => {
		const responseData = await this.EmployeeLeaveService.getAllEmployeeLeaveTypes(req.query, req.user as User);
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});

	public findEmployeeLeavePdfData = catchAsync(async (req: Request, res: Response) => {
		const id = req.params.id;
		const responseData = await this.EmployeeLeaveService.getEmployeeLeavePdfData(+id);
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});

	public findEmployeeLeaveById = catchAsync(async (req: Request, res: Response) => {
		const id = req.params.id;
		const responseData = await this.EmployeeLeaveService.getEmployeeLeaveById(+id);
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});
	...
	public addEmployeeLeave = catchAsync(async (req: Request, res: Response) => {
		...
		try {
			...
			const finalErrorMessage = errorPart ? errorPart.trim() : `Something went Wrong !!`;
			await transaction.rollback();
			return generalResponse(req, res, error, finalErrorMessage, 'error', true, 400);
		}
	});

	public updateEmployeeLeave = catchAsync(async (req: Request, res: Response) => {
		const transaction = await db.transaction();
		const id = req.params.id;
	...
	public updateEmployeeLeaveStatus = catchAsync(async (req: Request, res: Response) => {
		const transaction = await db.transaction();
		const id = req.params.id;
	...
	public findEmployeeLastLeaveByEmployeeId = catchAsync(async (req: Request, res: Response) => {
		const id = req.params.employeeId;
		const responseData = await this.EmployeeLeaveService.getEmployeeLastLeaveByEmployeeId(+id);
		return generalResponse(req, res, responseData, this.msg.fetch, 'success', false);
	});
}
