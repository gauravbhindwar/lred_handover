...
class UserController {
	...
	public addUser = catchAsync(async (req: Request, res: Response) => {
		...
		const isExistEmail = await LoginUser.findOne({ where: { email: body.email, deletedAt: null } });
		...
		// 	attributes: ['id', 'name'],
		// });
		// if (isExistEmail && isEmployeeRole?.name === 'Employee') {
		...
		// let loginUserData;
		// if (isExistEmail) {
		// 	loginUserData = isExistEmail;
		// 	const employeeRole = await Role.findOne({ where: { name: 'Employee' } });
		// 	const existUser = await User.findOne({
		// 		where: { loginUserId: isExistEmail.id, roleId: employeeRole.id },
		// 		include: [{ model: Role }],
		// 	});
		// 	if (existUser) {
		// 		const userDefaultPermission = await UserPermission.findAll({
		// 			where: { loginUserId: isExistEmail.id, roleId: employeeRole.id },
		// 		});
		// 		if (userDefaultPermission.length) {
		// 			const oldPermission = userDefaultPermission.map((permission) => permission.permissionId);
		// 			const newPermission = [...new Set([...(body?.permissions ?? []), ...oldPermission])];
		// 			body = { ...body, permissions: newPermission };
		// 		}
		// 	}
		// } else {
	 ...
	 * @returns {Promise<void>}
	 */
	// AND "roleData"."name" != 'super admin')

	public getUsers = catchAsync(async (req: Request, res: Response) => {
		const { page, limit, isActive, clientId, search, roleId, segmentId, subSegmentId, listView }: IQueryParameters =
			req.query;
		let responseObj;
		const isEmployeeRole = await Role.findOne({
			where: {
		...
		if (_.isUndefined(listView)) {
			...
			if (result[0].length) {
				for (const userList of result[0] as any[]) {
					...
					if (resultPermission?.[0]?.length) userList['loginUserData.assignedUserPermission'] = resultPermission[0];
				}
			}
			...
			responseObj = {
				...
				lastPage: page && limit ? Math.ceil(Number(userCount) / +limit) : undefined,
			};
		} else {
			const fromClause = `FROM "users" AS "User" 
            INNER JOIN "login_user" AS "loginUserData" ON "User"."loginUserId" = "loginUserData"."id" 
		...
		return generalResponse(req, res, responseObj, this.msg.fetch, 'success', false);
	});

	public getUsersForSearchDropdown = catchAsync(async (req: Request, res: Response) => {
		const { roleId, isActive, clientId, segmentId, subSegmentId }: IQueryParameters = req.query;
		let segmentValue;
		let subSegmentValue;
		...
		const responseData = await this.userRepository.getAll({
			where: {
				...
				...(isActive != undefined && { status: isActive.toString() === 'true' ? status.ACTIVE : status.INACTIVE }),
				...(clientId && {
					[Op.or]: {
		...
		return generalResponse(req, res, dropdownData, this.msg.fetch, 'success', false);
	});
		...
		let responseData = await this.userRepository.getAll({
			...
			include: [
				...
				},
				{
					model: UserSegment,
					required: false,
					where: { ...(Number(clientId) ? { clientId: clientId } : { clientId: null }) },
					attributes: ['id'],
					include: [
						{
							model: Segment,
	...
	public getUserRolePermission = catchAsync(async (req: Request, res: Response) => {
		...
		let responseData = await this.userRepository.get({
			...
			include: [
				...
				},
				{
					...
					required: false,
					attributes: ['id', 'clientId'],
					where: { userId: id, ...(Number(clientId) ? { clientId: clientId } : {}) },
					include: [
						{
							model: Client,
							attributes: ['id'],
				...
				},
				{
					model: UserSegment,
					required: false,
					where: { userId: id, clientId: Number(clientId) ? clientId : null, deletedAt: null },
					attributes: ['id'],
					include: [
						{
							model: Segment,
	...
	public getMessageUserById = catchAsync(async (req: Request, res: Response) => {
		// const { clientId } = req.query;
	...
	public updateUserById = catchAsync(async (req: Request, res: Response) => {
		...
		if (Number(clientId)) {
			const existUserClient = await this.userClientRepo.get({
				where: { userId: Number(req.params.id), clientId: Number(clientId), deletedAt: null },
			});
			if (existUserClient) {
				existUserClient.update({ roleId: body.roleId });
				const getUserData = await this.userRepository.get({ where: { id: existUserClient.userId } });
		...
		responseData = parse(responseData);
		if (responseData) {
			// const isEmployeeUserExist = await User.findAll({
			// 	where: { id: { [Op.ne]: responseData.id }, loginUserId: responseData.loginUserId },
			// });
			const password = body.password && (await bcrypt.hash(body.password, 10));
			await this.loginUserRepo.update(
				{
					name: body.name,
					timezone: body.timezone,
					isMailNotification: body.isMailNotification,
			...
			body.permissions &&
				(await Promise.all(
					body.permissions?.map(async (permission: number) => {
						...
						existUserPermissionList = [...existUserPermissionList, Number(permission)];
			...
			await UserPermission.destroy({
				where: {
					...
					[Op.or]: {
						...
						[Op.and]: { roleId: body.roleId, permissionId: { [Op.notIn]: existUserPermissionList } },
					},
					clientId: clientIds,
				},
			});
		...
		return generalResponse(req, res, {}, 'User updated successfully', 'success', true);
	});
	...
	public updateUserStatus = catchAsync(async (req: Request, res: Response) => {
		...
		if (Number(clientId)) {
			...
			if (existUserClient) {
				existUserClient.update({ status: existUserClient.status === status.ACTIVE ? status.INACTIVE : status.ACTIVE });
			}
		} else {
			...
			await this.userRepository.updateUser({
				data: {
					status: isExistUser.status === status.ACTIVE ? status.INACTIVE : status.ACTIVE,
				},
				action: 'update',
				id: Number(req.params.id),
			});
		}
		return generalResponse(req, res, {}, 'User status changed successfully', 'success', true);
	});

	public updateUser = catchAsync(async (req: Request, res: Response) => {
		const { file: files } = req;
		if (files) req.body = { ...req.body, profileImage: `/profile/${files.filename}` };
		...
		return generalResponse(req, res, responseData, 'User updated successfully', 'success', true);
	});

	public updateUserLoginUserDataById = catchAsync(async (req: Request, res: Response) => {
		const { clientId }: IQueryParameters = req.query;
	...
	public updateUserSegmentDataById = catchAsync(async (req: Request, res: Response) => {
		const { clientId }: IQueryParameters = req.query;
	...
	public removeUserClient = catchAsync(async (req: Request, res: Response) => {
		const { clientId }: IQueryParameters = req.query;
		await this.userClientRepo.deleteUserClient(Number(req.params.id), clientId);
		return generalResponse(req, res, {}, 'User Client Deleted Successfully', 'success', false);
	});

	public removeUserSegment = catchAsync(async (req: Request, res: Response) => {
		const { type } = req.query;
		await this.userSegmentRepo.deleteUserSegment(Number(req.params.id), type as string);
		return generalResponse(req, res, {}, 'User Segment Deleted Successfully', 'success', false);
	});
	...
	public sendLink = catchAsync(async (req: Request, res: Response) => {
		...
		return generalResponse(req, res, data, 'Email sent successfully', 'success', true);
	});
}
